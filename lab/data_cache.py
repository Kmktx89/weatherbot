...
class DataCache:
    def __init__(self, path: str | Path):
        ...
        # SQLite connections are not thread-safe for concurrent statement execution
        # even with check_same_thread=False. Worker threads in lab.inputs call
        # set() while the parent thread may still be holding a cursor open from
        # get(). Serialize every operation through this lock.
    ...
    def set(self, key: str, value: Any, *, source: str, target_date: str | None) -> None:
        ...
        with self._lock:
            self._conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?)",
                (key, payload, ts, source, target_date),
            )
